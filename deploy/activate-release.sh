...
# Runs as calkilo-web on the server; no sudo or Nginx reload is needed.
...
for path in / /contact/ /fa/ /robots.txt; do
  curl --fail --silent --show-error --max-time 15 \
    -H 'Host: calkilo.com' "http://127.0.0.1$path" -o /dev/null || healthy=false
done
actual=$(curl --fail --silent --show-error --max-time 15 \
  -H 'Host: calkilo.com' http://127.0.0.1/deployment.txt) || healthy=false
